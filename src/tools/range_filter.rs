...
pub fn check_address_range<T: FromStr + Copy + PartialOrd>(default_min: T, default_max: T, range_args: &Vec<String>, socket_field: &String) -> bool {
    ...
    match get_addr {
        Ok(addr) => {
            check_field_range(default_min, default_max, range_args, addr)
        }   
        Err(_) => {
            false
        }
        ...
        "local-address" => {
            if let Some(version) = ip_version {
                match version {
                    ...
                    6 => {
                        check_address_range(MIN_IPV6, MAX_IPV6, range_args, &socket.local_addr)
                    }
                    _ => {
                        false
                    }
            ...
            if let Some(addr) = &socket.remote_addr {
                if let Some(version) = ip_version {
                    match version {
                        ...
                        6 => {
                            check_address_range(MIN_IPV6, MAX_IPV6, range_args, addr)
                        }
                        _ => {
                            false
                        }
